...
import { ERole, IUserDTO } from "@/types/ILoginDTO";
...
export const useAuth = (): useAuthReturn => {
   ...
   * @async
   * @returns {Promise<AuthCheckReturn | null>} - Returns the user object or null if no session.
   */
  const checkAuth = async (): Promise<AuthCheckReturn | null> => {
    try {
      setLoading(true);
      const {
        data: { session },
   ...
   * @param {string} email - User's email address.
   * @param {string} password - User's password.
   * @returns {Promise<AuthCheckReturn | null>} - Returns user data if login is successful, otherwise null.
   */
  const login = async (
    email: string,
    password: string
